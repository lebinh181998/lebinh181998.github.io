...
class ShowOrder extends Component {
	...
	Done = (e)=>{
		...
		formData.append('table_id', tableID);
		    axios.post('http://127.0.0.1:8000/api/menu/updateorder2',
			formData,
            {headers: { 'content-type': 'multipart/form-data' }})
		    .then(res => {
		        var data = res.data;
		        if (data.status == 200) {
		            console.log(data);
					this.props.toggleOrder(tableID, 'false', data.orders[0]);
		        }
		    })
		    .catch(error => console.log(error));
	}
								...
								list_order.map((food, index) => {
									return (
										<li className='row m-0 pt-1' key={index}>
												...
												<div id={food.foodName} onClick={this.Called} className='w-25'>
													{list_order
														...
														(list_order2[index]
															...
															: <span id={food.foodID} onClick={this.Done} className="input-group-text p-0 px-2">
																{index + 1}
															</span>)
														: <span id={food.foodID} onClick={this.Done} className="input-group-text p-0 px-2">
															{index + 1}
											...
											<div className='col-md-12 form-change-order'>
												...
												{list_order2
													...
													(list_order2[index] 
														...
														&& food.note != list_order2[index].note ?
														<div className='row'>
															<span className='col-md-1 text-left'>
